...
#include "branch_len.h"
#include "branch_addr.h"
#include "offsets.h"
#include "map_b_instrs.h"

#define INSTR_OFFSET 0xb74
...
static int64_t branches[NUM_RANDS] = BRANCH_ADDRS;
...
int main() {
  ...
  // Get the offset of the address of the code section
  uint64_t instr_addr = 0;
  __asm__ volatile (
      "       adr %0, . \n"
      : "=r"(instr_addr) // Output variables
      :
      :
  );
  uint64_t code_offset = instr_addr - INSTR_OFFSET;

  // Get the address of the test branch instruction and copy it to the mapped page
  uint64_t map_b_instrs_addr = 0;
  __asm__ volatile (
      "       adr %0, map_b \n"
      : "=r"(map_b_instrs_addr) // Output variables
      :
      :
  );

  for (int i = 0; i < NUM_RANDS; i++) {
    // Map a page to the address of the test vranch
    uint8_t* map_addr = mmap((uint8_t*) (branches[i] + code_offset), BRANCH_LEN,
			     PROT_EXEC|PROT_READ|PROT_WRITE,
			     MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
    if (map_addr == MAP_FAILED) {
      perror("failed to mmap branch address");
      return EXIT_FAILURE;
    }
    
    memcpy((uint8_t*) (branches[i] + code_offset), (uint8_t*) map_b_instrs_addr, BRANCH_LEN);
  }
  ...
  __asm__ volatile (
      ...
      : "x1", "x2", "x3", "x5", "x6", "x10", "x11"   // Clobbered registers
  );
