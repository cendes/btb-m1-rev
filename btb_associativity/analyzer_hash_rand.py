...
for i in range(30, 47, 1):
    ...
    for _ in range(math.factorial(len(EVICTION_SET))):
        ...
        rand_set = list()
        rand_set.append(EVICTION_SET[0])
        for addr in EVICTION_SET[1:]:
            mask = (0x1 << i)
            rand_set.append(addr | mask)

        a_file = open("branch_addr.h", "w")
        a_file.write("#define BRANCH_ADDRS {" + ", ".join([hex(x - 44) for x in rand_set[1:]]) + "}\n")
        a_file.close()

        first_addr = rand_set[0] - 52
        branch_targets = list()
        branch_inits = list()
        first_target = rand_set[0] - 44
        branch_inits.append(rand_set[0] - 24)
        for branch_addr in rand_set[1:]:
            branch_inits.append(branch_addr - 24)
            branch_targets.append(branch_addr - 44)
        branch_targets.append(rand_set[0] + 4)
        ...
        o_file.write("#define NUM_RANDS " + str(len(rand_set) - 1) + "\n")
        o_file.close()
        ...
        if num_misses is not None:
            print("Number of misses for " + str([hex(x) for x in rand_set]) + ": " + str(num_misses))
            if num_misses > highest_misses:
                highest_misses = num_misses
                optimal_set = copy.deepcopy(rand_set)
