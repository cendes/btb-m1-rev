...
while True:
    ...
    rand_branches.append(curr_branch)
    ...
    for _ in range(NUM_BRANCHES-1):
        upper_bits = random.getrandbits(14)
        curr_branch = new_test_branch | (upper_bits << 32)
        branches.append(curr_branch)
        rand_branches.append(curr_branch)
    ...
    a_file.write("#define BRANCH_ADDRS {" + ", ".join([hex(x - 44) for x in rand_branches]) + "}\n")
    a_file.close()
    ...
    o_file.write("#define NUM_RANDS " + str(len(rand_branches)) + "\n")
    o_file.close()
    ...
    if num_misses is None:
        continue

    print("Number of misses for " + str([hex(x) for x in branches]) + ": " + str(num_misses))